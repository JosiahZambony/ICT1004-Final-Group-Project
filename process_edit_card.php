    ...
    <body>
        ...
        <section>
            ...
            if (!$success) {
                ...
                . $error_msg
                . "</ul>"
                . "<button class='btn btn-outline-dark' type='submit'>Return to Edit card</button>"
                . "</form>";
            }else{
            ...
            function edit_card_to_db() {
                ...
                if ($conn->connect_error) {
                    ...
                    $stmt = $conn->prepare("UPDATE cards_info SET name=?, quantity=?, picture_link=? WHERE name = ?");

                    // Bind & Execute the query statement:
                    $stmt->bind_param("siss", $cardname, $qty, $link,$cardname);
                    if (!$stmt->execute()) {
